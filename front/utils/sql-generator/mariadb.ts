 ...
 * MariaDB SQL 生成器
 *
 * 生成 MariaDB DDL 语句，与 MySQL 高度兼容，
 * 但支持 MariaDB 特有的类型和语法。
 */
...
function generateFieldSQL(field: TableField): string {
  const typeDef = getDataType('mariadb' as DatabaseType, field.type)
  ...
  // DEFAULT
  if (field.default !== '') {
    // UUID 类型特殊处理：UUID() 函数不需要引号
    if (field.type === 'UUID' && field.default.toUpperCase() === 'UUID()') {
      def += ' DEFAULT UUID()'
    } else {
