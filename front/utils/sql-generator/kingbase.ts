 ...
 * 生成 KingBase DDL
 */
export function toKingBase(diagram: DiagramData): string {
...
function parseTypeSize(field: TableField): string {
  const typeDef = getDataType('kingbase' as DatabaseType, field.type)
...
function generateFieldSQL(field: TableField): string {
  ...
  if (field.default !== '' && !field.increment) {
    def += ` DEFAULT ${parseDefault(field, 'kingbase')}`
  }
