 ...
 *
 * Dialog for creating a new database table with inline editing for all fields.
 */

import { useState, useEffect } from 'react'
import * as Dialog from '@radix-ui/react-dialog'
import { X, Plus, ChevronDown, ChevronUp, Save, Trash2 } from 'lucide-react'
...
export function CreateTableDialog({
  ...
  const generateSql = (): string => {
    ...
    const pkCols = validColumns.filter((c) => c.isPrimaryKey).map((c) => `\`${c.name}\``)
    ...
    indexes.filter((i) => i.columns.length > 0).forEach((idx) => {
      ...
      defs.push(`  ${indexType} \`${idx.name}\` (${colList}) USING ${idx.indexType}`)
    })

    foreignKeys.filter((f) => f.column && f.refTable && f.refColumn).forEach((fk) => {
      defs.push(`  CONSTRAINT \`${fk.name}\` FOREIGN KEY (\`${fk.column}\`) REFERENCES \`${fk.refTable}\`(\`${fk.refColumn}\`) ON DELETE ${fk.onDelete} ON UPDATE ${fk.onUpdate}`)
    })

    checkConstraints.filter((c) => c.expression).forEach((chk) => {
      defs.push(`  CONSTRAINT \`${chk.name}\` CHECK (${chk.expression})`)
    })
    ...
    triggers.filter((t) => t.statement).forEach((trg) => {
      sql += `\n\nCREATE TRIGGER \`${trg.name}\` ${trg.timing} ${trg.event} ON \`${database}\`.\`${tableName}\` FOR EACH ROW ${trg.statement}`
    })
        ...
        >
          ...
          <div className={cn('flex items-center justify-between px-4 py-3 border-b shrink-0', borderColor)}>
            <Dialog.Title className={cn('text-base font-medium', textPrimary)}>
              {t('database.createTable')} - {database}
            </Dialog.Title>
