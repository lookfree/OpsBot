/**
 * Edit Table Structure Dialog Component
 *
 * Dialog for editing an existing table's structure with inline editing.
 */

import { useState, useEffect } from 'react'
import * as Dialog from '@radix-ui/react-dialog'
import { X, Plus, ChevronDown, ChevronUp, Loader2, Save, Trash2 } from 'lucide-react'
...
interface EditTableStructureDialogProps {
  open: boolean
...
export function EditTableStructureDialog({
  ...
  useEffect(() => {
    if (open && connectionId && database && tableName) {
      resetState()
      loadTableStructure()
    }
  ...
  const generateAlterStatements = (): string[] => {
    const stmts: string[] = []
    const tbl = `\`${database}\`.\`${tableName}\``

    // Drop indexes first
    indexes.filter(i => i.isDeleted && !i.isNew && i.originalName !== 'PRIMARY').forEach(i =>
      stmts.push(`ALTER TABLE ${tbl} DROP INDEX \`${i.originalName}\`;`))
    ...
    columns.filter(c => c.isModified && !c.isNew && !c.isDeleted).forEach(c => {
      let def = `\`${c.name}\` ${c.type}${c.length ? `(${c.length})` : ''}`
      ...
      stmts.push(c.name !== c.originalName
        ? `ALTER TABLE ${tbl} CHANGE COLUMN \`${c.originalName}\` ${def};`
    ...
    indexes.filter(i => i.isNew && !i.isDeleted && i.columns.length > 0).forEach(i => {
      ...
      stmts.push(`ALTER TABLE ${tbl} ADD ${type} \`${i.name}\` (${i.columns.map(c => `\`${c}\``).join(', ')}) USING ${i.indexType};`)
    })

    // Drop foreign keys
    foreignKeys.filter(f => f.isDeleted && !f.isNew).forEach(f =>
      stmts.push(`ALTER TABLE ${tbl} DROP FOREIGN KEY \`${f.originalName}\`;`))

    // Add foreign keys
    foreignKeys.filter(f => f.isNew && !f.isDeleted && f.column && f.refTable && f.refColumn).forEach(f =>
      stmts.push(`ALTER TABLE ${tbl} ADD CONSTRAINT \`${f.name}\` FOREIGN KEY (\`${f.column}\`) REFERENCES \`${f.refTable}\`(\`${f.refColumn}\`) ON DELETE ${f.onDelete} ON UPDATE ${f.onUpdate};`))

    // Drop check constraints
    checkConstraints.filter(c => c.isDeleted && !c.isNew).forEach(c =>
      stmts.push(`ALTER TABLE ${tbl} DROP CHECK \`${c.originalName}\`;`))

    // Add check constraints
    checkConstraints.filter(c => c.isNew && !c.isDeleted && c.expression).forEach(c =>
      stmts.push(`ALTER TABLE ${tbl} ADD CONSTRAINT \`${c.name}\` CHECK (${c.expression});`))

    // Drop triggers
    triggers.filter(t => t.isDeleted && !t.isNew).forEach(t =>
      stmts.push(`DROP TRIGGER IF EXISTS \`${database}\`.\`${t.originalName}\`;`))

    // Add triggers
    triggers.filter(t => t.isNew && !t.isDeleted && t.statement).forEach(t =>
      stmts.push(`CREATE TRIGGER \`${t.name}\` ${t.timing} ${t.event} ON ${tbl} FOR EACH ROW ${t.statement}`))
  ...
  const handleSave = async () => {
    ...
    if (stmts.length === 0) { onOpenChange(false); return }
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className={cn(
          ...
          'rounded-lg shadow-xl z-50 w-[1000px] h-[650px] flex flex-col p-0 outline-none border',
          dialogBg, borderColor
        )}>
          {/* Title */}
          <div className={cn('flex items-center justify-between px-4 py-3 border-b shrink-0', borderColor)}>
            <Dialog.Title className={cn('text-base font-medium', textPrimary)}>
              {t('database.editTableStructure')} - {tableName}
            </Dialog.Title>
